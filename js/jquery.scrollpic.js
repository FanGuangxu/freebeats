    ...
    $.fn.ScrollPic = function (option) {
        return this.each(function () {
            ...
            if (opts.numbercontrol) {
                ...
                for (i = 1; i <= len; i++) {
//                  PicObject.find('.yiz-page-btn').append('<span>' + i + '</span>')//正方形，带数字
                    PicObject.find('.yiz-page-btn').append('<span></span>')//圆点
                }
            ...
            function isTouchDevice() {  
                try{  //phone
                    ...
                    PicObject.on('touchend',function(e){
                        if (opts.autoscrooll)time = setInterval(picTimer, opts.Time)
                    })
